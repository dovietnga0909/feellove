...
function _load_tour_accommodation_rates($tour_id, $check_rate_info){

	$CI =& get_instance();
	
	$startdate = format_bpv_date($check_rate_info['startdate']);
	
	$enddate = format_bpv_date($check_rate_info['enddate']);
	
	$departure_id = isset($check_rate_info['departure_id']) ? $check_rate_info['departure_id'] : null;
	
	$accommodations = $CI->Tour_Model->get_accommodations($tour_id);
	
	$accommodation_rates = $CI->Tour_Model->get_accommodation_rates($tour_id, $startdate, $enddate, $departure_id);

	$tour_promotions = $CI->Tour_Model->get_tour_promotions($tour_id, $startdate, $enddate, $departure_id);
	
	$default_cancellation = $CI->Tour_Model->get_cancellation_of_tour($tour_id, false);
...
function _calculate_tour_promotion_rate($basic_rate, $pro, $departure_date, $accommodation, $num_adult, $num_children, $num_infant)
{
    ...
    foreach ($pro['accommodations'] as $value)
    {
        ...
        if ($value['accommodation_id'] == $accommodation['id'] && ! empty($value['get']))
        {
            if ($pro['discount_type'] == DISCOUNT_TYPE_AMOUNT_PER_PAX)
            {
                $accommodation['adult_rate'] = bpv_round_rate($accommodation['adult_rate'] - $value['get']);
                
                $accommodation['single_sup_rate'] = !empty($accommodation['single_sup_rate']) ? bpv_round_rate($accommodation['single_sup_rate'] - $value['get']) : 0;
            }
            else
            {
