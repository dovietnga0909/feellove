	...
	<form role="form" id="hotel_search_form" name="hotel_search_form" method="post" action="<?=get_url(HOTEL_SEARCH_PAGE)?>">
		...
		<div class="row">
			...
			<div class="col-xs-2">
				...
				<span class="icon-after">
	    			...
	    			<input type="text" class="form-control <?=$warning_class?>" name="startdate" id="startdate" maxlength="10" 
	    				value="<?= !empty($search_criteria['startdate']) && check_bpv_date($search_criteria['startdate']) ? $search_criteria['startdate'] : ''?>" autocomplete="off" <?php if($is_tablet):?>readonly="readonly"<?php endif;?>>
			...
			<div class="col-xs-2">
				...
				<div>
					<input type="hidden" id="enddate" name="enddate" value="<?= !empty($search_criteria['enddate']) ? $search_criteria['enddate'] : ''?>">
