...
<div class="bpv-list margin-top-20">
    ...
    <?php foreach ($s_tours as $k => $tour):?>
        ...
        <div class="bpv-item" onclick="go_url('<?=cruise_tour_build_url($tour, $search_criteria)?>')">
                
            <img class="pull-left" src="<?=get_image_path(CRUISE_TOUR, $tour['picture'],'160x120')?>">
        
            <div class="item-name"><?=$tour['name']?> <i class="icon star-<?=str_replace('.', '_', $tour['star'])?>"></i></div>
