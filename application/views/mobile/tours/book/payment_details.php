<div class="bpv-total-payment margin-bottom-20">
	...
	<div class="content">
		...
		<?php if(!empty($payment_detail['surcharges'])):?>
	...
	<div class="pro-code">
		<div class="p-row clearfix" style="border-bottom:0">
			...
			<div class="row pro_phone_block" style="display: none; margin-bottom: 5px;">
			...
			<div class="row">
				...
				<div class="col-xs-6 text-right">
    				...
    				<input type="hidden" value="<?=$tour['id']?>" name="pro_tour" id="pro_tour">
    				<?php 
    					$nr_passengers = $check_rate_info['adults'];
    				?>
    				<input type="hidden" value="<?=$nr_passengers?>" name="nr_pax" id="nr_pax">
    				<button class="btn btn-bpv btn-book-now btn-sm margin-top-10" id="pro_use" style="display:none" onclick="use_pro_code(<?=TOUR?>);return false;" data-loading-text="<?=lang('bpv_pro_code_loading')?>">
    					<?=lang('bpv_pro_use')?>
    				</button>
				</div>
			</div>
