...
<?php 
	$c_titles = $this->config->item('c_titles');
	$customer['title_text'] = lang($c_titles[$customer['gender']]);
?>

<div style="width:800px;margin-bottom:10px">
    <p>Xin chào quý khách <?=$customer['full_name']?>,</p>
    <p>Đơn đặt tour của quý khách đã được gửi tới <a href="<?=site_url()?>"><?=SITE_NAME?></a>. Nhân viên của chúng tôi sẽ tiến hành đặt tour và liên hệ tới quý khách trong khoảng thời gian sớm nhất.
...
<table style="width:800px; border-spacing:5px;font-family: Arial; font-size:12px">
    	...
    	<td colspan="2" style="padding-top:10px">
    		<table style="margin-left: 30px; border-spacing:0;empty-cells:show;border-top: 1px solid #DDD;border-left:1px solid #DDD;font-family: Arial; font-size:12px;width: 750px">
				<thead style="font-size: 110%;font-weight: bold;background-color:#F0F0F0;color:#555;">
					<tr>
						<th align="left" width="50%" style="border-bottom:1px solid #DDD;padding:3px 7px;border-right:1px solid #DDD;"><?=lang('selected_class_name')?></th>
    ...
    <tr>
    	<td colspan="2">
    	<br>
        <span style="font-size:13px; font-weight:bold; color:#36C">Quy định của <?=$tour['name']?></span>
		<hr style="background:#39C; height:1px; border:0px;">
        </td>
    </tr>
    ...
    <tr>
        ...
        <td>
        	<table style="width: 100% ;background-color:#F0F0F0;border-spacing:0;empty-cells:show;font-family: Arial; font-size:12px; margin:5px 0">
        		<tr>
        			<td width="30%" style="padding:10px;border:1px solid #FFF">
        				Em bé (dưới <?=$tour['infant_age_util']?> tuổi
        			</td>
        		...
        		<tr>
        			<td style="padding:10px;border:1px solid #FFF">
        				Trẻ em (<?=$tour['infant_age_util']?> đến <?=$tour['children_age_to']?> tuổi)
        			</td>
        			<td style="padding:10px;border:1px solid #FFF">
        				<?=$tour['children_policy']?>
        			</td>
        		</tr>
        	</table>
        	<p style="margin-bottom: 10px">(*) Trẻ em trên <?=$tour['children_age_to']?> tuổi được coi là người lớn</p>
        </td>
    </tr>
    
    <tr>
        <td valign="top" style="padding-left: 30px;"><b><?=lang('tour_important_info')?></b></td>
        <td>
        	<?php
            	$notes = $tour['notes'];
            ...
            <?php if(count($notes) > 0):?>
            	...
            	<ul style="list-style: none; padding-left: 0">
            		<?php foreach ($notes as $value):?>
            			<?php if(!empty($value)):?>
            			<li style="margin-bottom: 8px"><?=$value?></li>
            			<?php endif;?>
            		<?php endforeach;?>
            	</ul>
            ...
            <div style="margin-top: 20px">
                <h4 style="color: #ba1c1d"><?=lang('tour_outbound_note_title')?></h4>
