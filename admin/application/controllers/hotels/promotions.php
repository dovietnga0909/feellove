...
class Promotions extends BP_Controller {
	...
	public function __construct()
    {
		...
		$this->load->helper(array('url','form','promotion','hotel'));
	...
	public function index()
	{
		...
		$data['search_frm'] = $this->load->view('hotels/promotions/search_promotion', $data, TRUE);
		
		$data['content'] = $this->load->view('hotels/promotions/list_promotion', $data, TRUE);
	...
	public function _set_common_data($data = array()){
		...
		$this->session->set_userdata('MENU', MNU_HOTEL_PROMOTION);
		...
		$data['pro_room_types'] = $this->config->item('pro_room_types');
	
		$data['cancellations'] = $this->Cancellation_Model->get_all_cancellations(HOTEL);
		
		$data['hotel_room_types'] = $this->Promotion_Model->get_hotel_room_types_4_pro($hotel_id);
	...
	public function _build_search_criteria(){
		...
		if(empty($submit_action)){			
			...
			$search_criteria = $this->session->userdata(PROMOTION_SEARCH_CRITERIA.$hotel_id);
			...
			$this->session->set_userdata(PROMOTION_SEARCH_CRITERIA.$hotel_id, $search_criteria);
	...
	public function create($step){
		...
		if($action == 'next'){
			...
			if($next_status){
				redirect(site_url('hotels/promotions/'.$hotel_id).'/create/'.($step + 1).'/');
			}
		}
		...
		if($action == 'save'){
			...
			if($save_status){
				...
				redirect(site_url('hotels/promotions/'.$hotel_id).'/');
		...
		$data['pro_step'] = $this->load->view('hotels/promotions/step_promotion', $data, TRUE);
		
		$data = get_library('tinymce', $data);
		
		$data['content'] = $this->load->view('hotels/promotions/promotion_'.$step, $data, TRUE);		
		$this->load->view('_templates/template', $data);
	}
	...
	public function edit($id, $step = 0){
		...
		if($action == 'next'){
			...
			if($next_status){
				redirect(site_url('hotels/promotions/'.$hotel_id).'/edit/'.$id.'/'.($step + 1).'/');
			}
		}
	...
	public function view($id){
		...
		$data['content'] = $this->load->view('hotels/promotions/promotion_5', $data, TRUE);		
		$this->load->view('_templates/template', $data);
	...
	public function _get_post_data($step, $pro = array()){
		...
		if($step == 4){
			...
			$pro_room_types = $this->input->post('pro_room_types');
			
			if(!empty($pro_room_types)){
				
				$rt_p_array = array();
				
				foreach ($pro_room_types as $value){					
					$offer_note = $this->input->post('offer_'.$value);
					
					$get = $this->input->post('get_'.$value);
					
					$rt_p['room_type_id'] = $value;					
					$rt_p['offer_note'] = $offer_note;
					
					// only show get in room type
					if(is_show_room_type_get($pro) && strval($get) != ''){
						$rt_p['get'] = $get;
					} else {
						unset($rt_p['get']);
					}
	...
	public function _set_paging_info($data = array()){
		...
		$paging_config = get_paging_config($total_rows,'/hotels/promotions/'.$hotel_id.'/',PAGING_SEGMENT + 2);		
