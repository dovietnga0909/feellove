...
<div class="form-group">
	...
	<div class="col-xs-2">
		<select class="form-control" name="customer_type">
			...
			<option value="<?=$k?>" <?=set_select('customer_type', $k)?>><?=lang($type)?></option>
			<?php endforeach;?>
		</select>
...
<div class="form-group">
	...
	<div class="col-xs-2">
		<select class="form-control" name="customer_city">
			...
			<option value="<?=$city['id']?>" <?=set_select('customer_city', $city['id'])?>><?=$city['name']?></option>
			<?php endforeach;?>
		</select>
...
<div class="form-group">
	...
	<div class="col-xs-4">
		<?php if(isset($cruise)):?>
			...
			<?=form_error('tour_id')?>
		<?php else:?>
			<label class="control-label"><?=$review_for['name']?></label>
			<input type="hidden" id="tour_id" name="tour_id" value="<?=set_value('tour_id', $review_for['tour_id'])?>">
		<?php endif;?>
		<input type="hidden" id="hotel_id" name="hotel_id" value="<?=set_value('hotel_id', $review_for['hotel_id'])?>">
	</div>
</div>
<div class="form-group">
	...
	<div class="col-xs-4">
		<table class="table">
						...
						<tr>
							...
							<td>
								<input type="text" size="12" autocomplete="off" maxlength="3" style="text-align: right;" 
									name="<?=$key_type?>_<?=$key?>" id="<?=$key_type?>_<?=$key?>" onkeyup="totalScore();" value="<?=set_value($key_type.'_'.$key)?>">
			...
			<tr>
				...
				<td>
					<input type="text" size="12" maxlength="1" style="text-align: right;" name="total_score" id="total_score" readonly="readonly" value="<?=set_value('total_score')?>">
				</td>
			</tr>	
		</table>
	</div>
...
<div class="form-group">
	...
	<div class="col-xs-9">
		<textarea class="form-control" rows="9" name="review_content"><?=set_value('review_content')?></textarea>
