<ul class="nav nav-tabs mg-bottom-20">
	<li <?php if($group_id == 0) echo 'class="active"'?>>
		<a href="<?=site_url('/hotels/facilities/'.$hotel['id'])?>">
		<?=lang('hotels_field_all_facilities')?>
		<span class="badge"><?=$facility_count[0]?></span>
		</a>
	</li>
	<?php foreach ($facility_groups as $k => $group):?>
	<li <?php if($group_id == $k) echo 'class="active"'?>>
		<a href="<?=site_url('/hotels/facilities/'.$hotel['id'].'?g_id='.$k)?>">
		<?=lang($group)?>
...
<div class="panel panel-default">
	...
	<div class="panel-heading">
	  	...
	  	<?php endif;?>
	  	<a class="btn btn-primary btn-xs pull-right" href="<?=site_url('hotels/create_facility/'.$hotel['id'])?>" role="button">
		...
		<tbody>
			<?php if(!empty($facilities)):?>
				...
				<tr class="<?php echo  $facility['avaiable'] == STATUS_AVAIABLE ? 'avb' : 'not-avb';?>">
					<td>
						<?php if($facility['hotel_id'] == $hotel['id']):?>
						<a href="<?=site_url('hotels/edit_facility/'.$facility['id'])?>" title="<?=lang('ico_edit')?>">
							<?=$facility['name']?>
						</a>
						<?php else:?>
							<?=$facility['name']?>
					...
					<td class="text-right">
						<?php 
							$params = 'h_id='.$hotel['id'].'&f_id='.$facility['id'];
							if(!empty($group_id)) $params .= '&g_id='.$group_id;
						?>
						<?php if($facility['avaiable'] == STATUS_AVAIABLE):?>
							<a href="javascript:void(0)" data-url="<?=site_url('/hotels/facilities/update/?'.$params)?>" 
