<?php if(empty($sur)):?>
	<div class="alert alert-warning">
		...
		<a class="btn btn-primary mg-left-10" href="<?=site_url('hotels/surcharges/'.$hotel_id.'/')?>" role="button">
			<?=lang('btn_go_back')?>
...
<form class="form-horizontal" role="form" method="post">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-6">
      <input type="text" class="form-control" id="name" placeholder="<?=lang('surcharge_name')?>..." name="name" value="<?=set_value('name', $sur['name'])?>">
      <?=form_error('name')?>
    </div>
  </div>
    ...
    <div class="col-sm-3" id="start_date">
      	...
      	<div class="input-append date input-group">			    
			<input type="text" class="form-control" placeholder="<?=DATE_FORMAT_CALLENDAR?>..." id="start_date_ip" name="start_date" 
	    		value="<?=set_value('start_date', date(DATE_FORMAT, strtotime($sur['start_date'])))?>">
    ...
    <div class="col-sm-3" id="end_date">      
       <div class="input-append date input-group">			    
			<input type="text" class="form-control" placeholder="<?=DATE_FORMAT_CALLENDAR?>..." id="end_date_ip" name="end_date" 
	    		value="<?=set_value('end_date', date(DATE_FORMAT, strtotime($sur['end_date'])))?>">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-6">
    	...
    	<label class="checkbox-inline">    		
  			<input type="checkbox" id="check_box_<?=$key?>" value="<?=$key?>" <?=set_checkbox('week_day[]',$key, is_bit_value_contain($sur['week_day'], $key))?> name="week_day[]" > <?=lang($value)?>
		</label>
		<?php endforeach;?>
		<br>
	    <?=form_error('week_day[]')?>
    ...
    <div class="col-sm-3">
      ...
      <select class="form-control" id="charge_type" name="charge_type">
		  ...
		  <?php foreach ($charge_types as $key=>$value):?>
		  		<option value="<?=$key?>" <?=set_select('charge_type',$key, $key == $sur['charge_type'])?>><?=$value?></option>
		  <?php endforeach;?>		
	  </select>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-3">
      <input type="text" class="form-control" id="amount" placeholder="<?=lang('sur_field_amount')?>..." name="amount" value="<?=set_value('amount', number_format($sur['amount']))?>">      
      <?=form_error('amount')?>
    </div>
  </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-6">
      <textarea class="form-control rich-text" rows="5" name="description" id="description" placeholder="<?=lang('surcharge_description')?>..."><?=set_value('description', $sur['description'])?></textarea>
