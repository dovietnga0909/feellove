<div class="panel panel-default">
  ...
  <div class="panel-heading">
  	...
  	<a class="btn btn-primary btn-xs pull-right" href="<?=site_url('hotels/surcharges/'.$hotel_id.'/create')?>/" role="button">
		...
		<tbody>
			...
			<?php else:?>
				<?php foreach ($surcharges as $key=>$value):?>
					<tr>
						<td><?=$offset + $key + 1?></td>
						<td><a href="<?=site_url('hotels/surcharges/'.$hotel_id.'/edit/'.$value['id'].'/')?>"><?=$value['name']?></a></td>
						...
						<td><?=$value['amount']?> <span><?=lang('vnd')?></span></td>
						<td><?=get_last_update($value['date_modified'], $value['last_modified_by'])?></td>				
						<td>
							<a href="<?=site_url('hotels/surcharges/'.$hotel_id.'/edit/'.$value['id'].'/')?>">
								<span class="fa fa-edit"></span>
							</a>
							<a href="<?=site_url('hotels/surcharges/'.$hotel_id.'/delete/'.$value['id'].'/')?>" onclick="return confirm_delete('<?=lang('confirm_delete')?>')" class="mg-left-10">
