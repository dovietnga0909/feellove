		...
		<table class="table table-bordered">
			<thead>
				<tr>
					...
					<th colspan="2" class="text-center"><?=lang('rate_full_occupancy')?></th>
					<?php endif;?>
					
					<?php if($has_triple):?>
					<th colspan="2" class="text-center"><?=lang('rate_triple')?></th>
					<?php endif;?>
					
					<?php if($has_double):?>
					<th colspan="2" class="text-center"><?=lang('rate_double')?></th>
					<?php endif;?>
									
					<th colspan="2" class="text-center"><?=lang('rate_single')?></th>
			...
			<tbody>
				<?php foreach ($room_types as $value):?>
					...
					<?php 
						$rra = get_rra_by_room_type($value['id'], $hra['rras']);
						$full_occupancy_rate = !is_null($rra) ? number_format($rra['full_occupancy_rate']) : '';
						$triple_rate = !is_null($rra) ? number_format($rra['triple_rate']):'';
						$double_rate = !is_null($rra) ? number_format($rra['double_rate']):'';
						$single_rate = !is_null($rra) ? number_format($rra['single_rate']):'';
						$extra_bed_rate = !is_null($rra) ? number_format($rra['extra_bed_rate']):'';
						
						$full_occupancy_net = !is_null($rra) ? number_format($rra['full_occupancy_net']) : '';
						$triple_net = !is_null($rra) ? number_format($rra['triple_net']):'';
						$double_net = !is_null($rra) ? number_format($rra['double_net']):'';
						$single_net = !is_null($rra) ? number_format($rra['single_net']):'';
					?>
					...
					<tr>
						...
						<?php if($has_full_occupancy):?>
							...
							<?php if($value['max_occupancy'] > TRIPLE):?>	
								<td>
			    					...
			    					value="<?=set_value('rr_full_occupancy_'.$value['id'], $full_occupancy_rate)?>">
								</td>
								
								<td>
									<input type="text" class="form-control input-sm price-cell" placeholder="0000,000" name="net_rr_full_occupancy_<?=$value['id']?>" 
			    					value="<?=set_value('net_rr_full_occupancy_'.$value['id'], $full_occupancy_net)?>">
								</td>
							<?php else:?>
								<td colspan="2">&nbsp;</td>
							<?php endif;?>
						...
						<?php if($has_extra_bed):?>
							...
							<?php if($value['max_extra_beds'] > 0):?>
		    					...
		    					value="<?=set_value('rr_extra_bed_'.$value['id'], $extra_bed_rate)?>">
