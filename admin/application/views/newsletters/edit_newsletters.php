...
<form class="form-horizontal" role="form" method="post">
		...
		<div class="form-group">
			...
			<div class="col-xs-6">
				<input type="text" class="form-control" id="name" name="name" value="<?=set_value('name', $newsletters['name'])?>">
				<?=form_error('name')?>
			</div>
		</div>
			...
			<div class="col-xs-6">
				<select class="form-control" name="template_id" id="template_id">
					<?php foreach ($templates as $k => $temp):?>
						<option value="<?=$temp['id']?>" <?=set_select('template_id', $temp['id'], isset($newsletters['template_id']) && $newsletters['template_id'] == $temp['id']) ?> > <?=$temp['name']?></option>
					<?php endforeach;?>
				</select>
		...
		<div class="form-group">
			...
			<div class="col-xs-2" id="group_start_date">
				<div class="input-append date input-group">
					<input type="text" class="form-control" id="start_date" name="start_date" value="<?=set_value('start_date', date(DATE_FORMAT, strtotime($newsletters['start_date'])))?>" placeholder="<?=DATE_FORMAT_CALLENDAR?>...">
			...
			<div class="col-xs-2" id="group_end_date">
				<div class="input-append date input-group">
				<input type="text" class="form-control" id="end_date" name="end_date" value="<?=set_value('end_date', date(DATE_FORMAT, strtotime($newsletters['end_date'])))?>" placeholder="<?=DATE_FORMAT_CALLENDAR?>...">
				<span class="input-group-addon"><span class="fa fa-calendar"></span></span>
				</div>
				<?=form_error('end_date')?>
			</div>
		...
		<div class="form-group">
		    ...
		    <div class="col-xs-6">
		    	...
		    	<label class="checkbox-inline">
		    		<?php $checked = is_bit_value_contain($newsletters['send_on'], $key);?>
		    	
		  			<input type="checkbox" id="check_box_<?=$key?>" value="<?=$key?>" <?=set_checkbox('send_on[]', $key, $checked)?> name="send_on[]" > <?=lang($value)?>
				</label>
				<?php endforeach;?>
				<br>
			    <?=form_error('send_on[]')?>
		...
		<div class="form-group">
			...
			<div class="col-xs-2">
				<select class="form-control" name="send_hour" id="send_hour">
					...
					<option value="<?=$i?>" <?=set_select('send_hour', $i, $newsletters['send_hour'] == $i)?>> <?=$i?> </option>
					<?php endfor;?>
				</select>
			...
			<div class="col-xs-2">
				<select class="form-control" name="send_minute" id="send_minute">
					...
					<option value="<?=$value?>" <?=set_select('send_minute', $value, $newsletters['send_minute'] == $value)?>> <?=$value?> </option>
					<?php endforeach;?>
				</select>
		...
		<div class="form-group">
			...
			<div class="col-xs-6">
		    	...
		    	<label class="checkbox-inline">
		    		<?php $checked = is_bit_value_contain($newsletters['customer_gender'], $key);?>
		  			<input type="checkbox" id="check_box_<?=$key?>" value="<?=$key?>" <?=set_checkbox('customer_gender[]', $key, $checked)?> name="customer_gender[]" > <?=lang($value)?>
				</label>
				<?php endforeach;?>
				<br>
		...
		<div class="form-group">
			...
			<div class="col-xs-6">
		    	...
		    	<label class="checkbox-inline">
		    		<?php $checked = is_bit_value_contain($newsletters['customer_type'], $key);?>
		  			<input type="checkbox" id="check_box_<?=$key?>" value="<?=$key?>" <?=set_checkbox('customer_type[]', $key, $checked)?> name="customer_type[]" > <?=lang($value)?>
				</label>
				<?php endforeach;?>
				<br>
