...
<div class="form-group">
	...
	<div class="col-xs-10" style="padding-left: 0">
    	...
    	<div class="col-xs-3 checkbox">
    		...
    		<input type="checkbox" value="<?=$value['id']?>" name="tour_departures[]"
    			<?=set_checkbox('tour_departures[]', $value['id'])?>>
    		<?=$value['name']?>
    		</label>
        </div>
    	<?php endforeach;?>
	...
	<div class="col-xs-10">
        <?php foreach ($tour_departures as $value):?>
        <input type="hidden" value="<?=$value['id']?>" name="tour_departures[]" checked="checked">
        <?php endforeach;?>
...
<div class="form-group">
	...
	<div class="col-xs-10" style="padding-left: 0">
		<?php foreach ($tour_meals as $key=>$meal):?>
		    <div class="col-xs-3 checkbox">
        	    <label>
    			<input type="checkbox" value="<?=$key?>" name="meals[]" onclick="checkMeals(this)" <?=set_checkbox('meals[]', $key)?>>
    			<?=$meal?>
    			</label>
			</div>
		<?php endforeach;?>
...
<div class="form-group">
	...
	<div class="col-xs-10" style="padding-left: 0">
		<?php foreach ($tour_transportations as $key=>$value):?>
		    <div class="col-xs-3 checkbox">
        	    <label>
    			<input type="checkbox" value="<?=$key?>" name="transportations[]" <?=set_checkbox('transportations[]', $key)?>>
    			<?=$value?>
    			</label>
    		</div>
		<?php endforeach;?>
...
<div class="form-group">
	...
	<div class="col-xs-9">
		<textarea class="form-control rich-text" rows="8" name="accommodation"><?=set_value('accommodation')?></textarea>
...
<div class="form-group">
	...
	<div class="col-xs-9">
		<textarea class="form-control rich-text" rows="8" name="activities"><?=set_value('activities')?></textarea>
...
<div class="form-group">
	...
	<div class="col-xs-9">
		<textarea class="form-control rich-text" rows="8" name="notes"><?=set_value('notes')?></textarea>
		<?=form_error('notes')?>
	</div>
</div>
<div class="form-group">
	...
	<div class="col-xs-9">
		<input type="hidden" name="photos" id="itinerary_photos" value="<?=set_value('photos')?>">
		<a data-toggle="modal" href="<?=site_url('/tours/itinerary/photos/'.$tour['id'])?>" data-target="#myModal"><?=lang('tour_itineraries_select_photos')?></a>
	</div>
	<?=form_error('photos')?>
</div>
<div class="form-group">
    <div class="col-xs-offset-2 col-xs-6">
		...
		<a class="btn btn-default mg-left-10" href="<?=site_url('tours/itinerary/'.$tour['id'])?>" role="button"><?=lang('btn_cancel')?></a>
    </div>
</div>
