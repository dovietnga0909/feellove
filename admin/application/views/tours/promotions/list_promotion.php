<div class="panel panel-default">
  ...
  <div class="panel-heading">
  	...
  	<a class="btn btn-primary btn-xs pull-right" href="<?=site_url('tours/promotions/'.$tour_id.'/create')?>/" role="button">
		...
		<tbody>
			...
			<?php else:?>
				<?php foreach ($promotions as $key=>$value):?>
					<tr>
						<td><?=$offset + $key + 1?></td>
						<td><a href="<?=site_url('tours/promotions/'.$tour_id.'/edit/'.$value['id'].'/')?>"><?=$value['name']?></a></td>
						...
						<td>
							<a href="<?=site_url('tours/promotions/'.$tour_id.'/view/'.$value['id'].'/')?>">
								<span class="fa fa-list-alt"></span>
							</a>
							
							<a href="<?=site_url('tours/promotions/'.$tour_id.'/edit/'.$value['id'].'/')?>" class="mg-left-10">
								<span class="fa fa-edit"></span>
							</a>
							<a href="<?=site_url('tours/promotions/'.$tour_id.'/delete/'.$value['id'].'/')?>" onclick="return confirm_delete('<?=lang('confirm_delete')?>')" class="mg-left-10">
