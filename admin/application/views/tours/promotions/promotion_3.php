...
<script type="text/javascript">
    ...
    function change_discount_type(is_init){
		...
		if(discount_type == 2 || discount_type == 4 || discount_type == 5){ // amount discount per booking or free nights
    ...
    function change_text_label(is_init){
        ...
        if (discount_type == 2 || discount_type == 5){ // amount discount per booking
        	...
        	for(var i = 1; i <= 7; i++){
				...
				if(discount_type == 2) {
					...
					$('#label_get_' + i).text('<?=lang('pro_discount_amount_per_pax')?>');
				}
			}
