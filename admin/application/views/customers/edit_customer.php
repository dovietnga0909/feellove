<?php if(empty($customer)):?>
	<div class="alert alert-warning">
		...
		<a class="btn btn-primary mg-left-10" href="<?=site_url('customers')?>" role="button">
			<?=lang('btn_go_back')?>
...
<div class="form-group">
	...
	<div class="col-xs-2">
		<select class="form-control" name="gender">
			...
			<option value="<?=$k?>" <?=set_select('gender', $k, $k==$customer['gender'] ? true:false)?>>
			<?=lang($val)?>
			</option>
			<?php endforeach;?>
		</select>
...
<div class="form-group">
	...
	<div class="col-xs-4">
		<input type="text" class="form-control" name="full_name" value="<?=set_value('full_name', $customer['full_name'])?>">
...
<div class="form-group">
	...
	<div class="col-xs-3">
		<input type="text" class="form-control" name="phone" value="<?=set_value('phone', $customer['phone'])?>">
		<?=form_error('phone')?>
	</div>
</div>
<div class="form-group">
	...
	<div class="col-xs-4">
		<input type="text" class="form-control" name="email" value="<?=set_value('email', $customer['email'])?>">
		<?=form_error('email')?>
	</div>
</div>
...
<div class="form-group">
	...
	<select class="form-control" name="destination_id">
		...
		<option value="<?=$des['id']?>" <?=set_select('destination_id', $des['id'], $des['id']==$customer['destination_id'] ? true:false)?>><?=$des['name']?></option>
		<?php endforeach;?>
	</select>
...
<div class="form-group">
	...
	<div class="col-xs-6">
		<textarea class="form-control" rows="3" name="address"><?=set_value('address', $customer['address'])?></textarea>
		<?=form_error('address')?>
	</div>
</div>
<div class="form-group">
	...
	<div class="col-xs-6">
		<input type="checkbox" name="happy_or_not" value="1" <?=set_checkbox('happy_or_not', 1, 1==$customer['happy_or_not'] ? TRUE : FALSE)?>>
		<?=form_error('happy_or_not')?>
	</div>
</div>

<div class="form-group">
	<label for="birthday" class="col-xs-2 control-label"><?=lang('customers_field_birthday')?>:</label>
	<div class="col-xs-2" id="birthday">
		<div class="input-append date input-group">
		<input type="text" class="form-control" name="birthday" 
		value="<?=set_value('birthday', !empty($customer['birthday'])? date(DATE_FORMAT, strtotime($customer['birthday'])) : '')?>"
...
<div class="form-group">
	...
	<div class="col-xs-2">
		<select class="form-control" name="customer_budget">
			...
			<option value="<?=$k?>" <?=set_select('customer_budget', $k, $k==$customer['budget'] ? TRUE : FALSE)?>>
			<?=lang($val)?>
			</option>
			<?php endforeach;?>
		</select>
...
<div class="form-group">
	...
	<div class="col-xs-2">
		<select class="form-control" name="travel_types">
			...
			<option value="<?=$k?>" <?=set_select('travel_types', $k, $k==$customer['travel_type'] ? TRUE : FALSE)?>>
			<?=lang($val)?>
			</option>
			<?php endforeach;?>
		</select>
