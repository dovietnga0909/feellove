...
<div class="alert alert-warning">
	...
	<a class="btn btn-primary mg-left-10" href="<?=site_url('cruises/surcharges/'.$cruise_id.'/')?>" role="button"> <?=lang('btn_go_back')?>
...
<form class="form-horizontal" role="form" method="post">
		...
		<div class="col-sm-3" id="start_date">
			...
			<div class="input-append date input-group">
				<input type="text" class="form-control" placeholder="<?=DATE_FORMAT_CALLENDAR?>..." id="start_date_ip" name="start_date"
					value="<?=set_value('start_date', date(DATE_FORMAT, strtotime($sur['start_date'])))?>"> <span class="input-group-addon"><span
		...
		<div class="col-sm-3" id="end_date">
			<div class="input-append date input-group">
				<input type="text" class="form-control" placeholder="<?=DATE_FORMAT_CALLENDAR?>..." id="end_date_ip" name="end_date"
					value="<?=set_value('end_date', date(DATE_FORMAT, strtotime($sur['end_date'])))?>"> <span class="input-group-addon"><span
					class="fa fa-calendar"></span> </span>
			</div>
			<?=form_error('end_date')?>
		</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-6">
			...
			<?=set_checkbox('week_day[]',$key, is_bit_value_contain($sur['week_day'], $key))?> name="week_day[]"> <?=lang($value)?>
			</label>
			<?php endforeach;?>
			<br>
			<?=form_error('week_day[]')?>
	...
	<div class="form-group">
		...
		<div class="col-sm-3">
			...
			<select class="form-control" id="charge_type" name="charge_type">
				...
				<option value="<?=$key?>" <?=set_select('charge_type',$key, $key == $sur['charge_type'])?>>
					<?=$value?>
				</option>
				<?php endforeach;?>
			</select>
	...
	<div class="form-group">
		...
		<div class="col-sm-3">
			<input type="text" class="form-control money_input" id="adult_amount" placeholder="<?=lang('sur_field_adult_amount')?>..." name="adult_amount"
				value="<?=set_value('adult_amount', number_format($sur['adult_amount']))?>">
	...
	<div class="form-group">
		...
		<div class="col-sm-3">
			<input type="text" class="form-control money_input" id="children_amount" placeholder="<?=lang('sur_field_children_amount')?>..." name="children_amount"
				value="<?=set_value('children_amount', number_format($sur['children_amount']))?>">
	    ...
	    <div class="col-sm-3">      
	      <select class="form-control" id="apply_all" name="apply_all">		  
			  <?php foreach ($apply_on_tours as $key => $value):?>
			  	<option value="<?=$key?>" <?=set_select('apply_all',$key, isset($sur['apply_all']) && $sur['apply_all'] == $key)?>><?=$value?></option>
			  <?php endforeach;?>
		  </select>
	    </div>
	  </div>
	...
	<div id="cruise_tours" style="margin-bottom: 20px">
		...
		<?php
			$surcharge_tours = $sur['surcharge_tours'];
			$is_charge = false;
			$adult_amount = '';
			$children_amount = '';
			foreach ($surcharge_tours as $s_tour) {
				if($s_tour['tour_id'] == $tour['id']) {
					$is_charge = true;
					$adult_amount = $s_tour['adult_amount'];
					$children_amount = $s_tour['children_amount'];
				}
			}
		?>
		<div class="form-group">
			...
			<div class="col-sm-6">
				<div class="checkbox">
					...
					<input type="checkbox" name="surcharge_tours[]" class="chk_tours" value="<?=$tour['id']?>"
						<?=set_checkbox('surcharge_tours[]', $tour['id'], $is_charge)?>>
						<?=$tour['name']?>
					</label>
				</div>
			</div>
		...
		<div class="form-group hide" id="tour_ct_<?=$tour['id']?>">
			<div class="col-sm-3 col-sm-offset-2">
		    	<input type="text" class="form-control money_input" name="get_adult_<?=$tour['id']?>"
						value="<?=set_value('get_adult_'.$tour['id'], number_format($adult_amount))?>" style="width: 40%; float: left;">
		    	<span class="help-block">&nbsp;For Adult</span>
		    </div>
		    <div class="col-sm-3">
		    	<input type="text" class="form-control money_input" name="get_children_<?=$tour['id']?>"
						value="<?=set_value('get_children_'.$tour['id'], number_format($children_amount))?>" style="width: 40%; float: left;">
	...
	<div class="form-group">
		...
		<div class="col-sm-6">
			<textarea class="form-control rich-text" rows="5" name="description" id="description"
				...
				<?=set_value('description', $sur['description'])?>
			</textarea>
