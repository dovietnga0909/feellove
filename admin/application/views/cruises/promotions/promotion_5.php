...
<form class="form-horizontal" role="form" method="post">
  ...
  <!-- 
   ...
   -->
	  ...
	  <div class="form-group">
	    ...
	    <div class="col-sm-6">      
	      <?=$pro_cruise_tours[$pro['room_type']]?>
	    </div>
	  </div>
	  ...
	  <?php foreach ($cruise_tours as $tour):?>	  		
		  	...
		  	<div class="form-group">
			    ...
			    <div class="col-sm-4">
			  		<div class="checkbox">
					    <label>
					      <input type="checkbox" <?=set_checkbox('pro_tours[]', $tour['id'], is_tour_promotion($pro, $tour['id']))?> > <?=$tour['name']?>
					    </label>
					</div>
			    </div>
			  
			    <div class="col-sm-4">
			    	<span class="help-block"><?=get_tour_offer_note_pro($pro, $tour['id'])?></span>
			    </div>
			  </div>
			  
			  <?php foreach ($tour['accommodations'] as $acc):?>
  				
  					 <div class="form-group tour_acc_<?=$tour['id']?>">
					    <div class="col-sm-2 col-sm-offset-2">
					  		<label style="font-weight:normal;"><?=$acc['name']?></label>
					    </div>
					  
					    <div class="col-sm-2">
					    	<span class="help-block"><?=get_acc_get($pro, $acc['id'])?> <?=get_unit_label($pro, 0)?></span>
					    
					    </div>
					    
					    <div class="col-sm-4">
					    	<span class="help-block"><?=get_acc_offer_note($pro, $acc['id'])?></span>
					    </div>
  ...
  <div class="form-group">
    <div class="col-sm-offset-2 col-sm-6">
      ...
      <?php if(isset($view_mode)):?>
      <a class="btn btn-primary" href="<?=site_url('cruises/promotions/'.$cruise_id.'/edit/'.$pro['id'])?>" role="button"><span class="fa fa-edit"></span> <?=lang('btn_edit')?></a>
      <?php endif;?>
