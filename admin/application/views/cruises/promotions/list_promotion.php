<div class="panel panel-default">
  ...
  <div class="panel-heading">
  	...
  	<a class="btn btn-primary btn-xs pull-right" href="<?=site_url('cruises/promotions/'.$cruise_id.'/create')?>/" role="button">
		...
		<tbody>
			...
			<?php else:?>
				<?php foreach ($promotions as $key=>$value):?>
					<tr>
						<td><?=$offset + $key + 1?></td>
						<td><a href="<?=site_url('cruises/promotions/'.$cruise_id.'/edit/'.$value['id'].'/')?>"><?=$value['name']?></a></td>
						...
						<td><?=$pro_cruise_tours[$value['room_type']]?></td>
						
						<td><?=get_last_update($value['date_modified'], $value['last_modified_by'])?></td>				
						<td>
							<a href="<?=site_url('cruises/promotions/'.$cruise_id.'/view/'.$value['id'].'/')?>">
								<span class="fa fa-list-alt"></span>
							</a>
							
							<a href="<?=site_url('cruises/promotions/'.$cruise_id.'/edit/'.$value['id'].'/')?>" class="mg-left-10">
								<span class="fa fa-edit"></span>
							</a>
							<a href="<?=site_url('cruises/promotions/'.$cruise_id.'/delete/'.$value['id'].'/')?>" onclick="return confirm_delete('<?=lang('confirm_delete')?>')" class="mg-left-10">
